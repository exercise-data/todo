// 개인 세부 할일 (Firestore personalTasks CRUD) — 선택된 개인 프로젝트에 종속.
//
// personalTasks 문서: { ownerUid(=내 UID), projectId(=선택된 프로젝트 id),
//                       title, startDate("YYYY-MM-DD"), endDate, completed, createdAt }
//  - 추가: 제목 + 시작일 + 종료일. 제목 공백 또는 종료일<시작일이면 차단.
//  - 읽기: ownerUid==내 UID 이고 projectId==선택된 프로젝트인 것만 onSnapshot.
//  - 완료 체크 / 수정(제목·기간) / 삭제.
//  - 프로젝트 연쇄 삭제는 personalProjects.js(deleteProjectCascade)가 담당한다.
//
// 선택된 프로젝트는 personalProjects.js 가 "personal-project-selected" 커스텀 이벤트로 알려준다.
...
const SELECT_EVENT = "personal-project-selected";

// ----- DOM 참조 -----
const detailEl = document.querySelector(".pproj-detail");
const formEl = document.querySelector(".ptask-form");
const titleInput = document.querySelector(".ptask-title-input");
const startInput = document.querySelector(".ptask-start-input");
const endInput = document.querySelector(".ptask-end-input");
const errorEl = document.querySelector(".ptask-error");
const listToolbarEl = document.querySelector(".ptask-list-toolbar");
const listEl = document.querySelector(".ptask-list");
const ganttEl = document.querySelector(".ptask-gantt");
// 간트 도구막대 — 스크롤 박스(ganttEl) '밖'의 형제. 가로 스크롤에 딸려가지 않는다.
// ⚠️ 없으면 만들어 붙인다: 캐시 등으로 구버전 index.html(이 div 이전) 이 새 JS 와 함께 로드되면
//    여기서 null 이 되고, 아래 최상위 addEventListener 가 TypeError 를 내며 모듈 실행이 통째로
//    중단된다 → 프로젝트 선택/로그인 리스너가 등록되지 않아 has-project 가 영영 안 켜지고
//    세부 할 일(폼·목록·간트·토글)이 전부 사라진 채 "왼쪽에서 프로젝트를 선택하세요"만 남는다.
const ganttToolbarEl =
  document.querySelector(".ptask-gantt-toolbar") ||
  (() => {
...
const viewTabsEl = document.querySelector(".ptask-viewtabs");

// ----- 상태 -----
let currentUid = null;
let currentProjectId = null;
let unsubscribe = null;
let subKey = null; // 현재 구독 키("uid|projectId") — 중복 구독 방지
let tasksCache = [];
let editingId = null;
...
// 간트 자동 표시 범위의 앞뒤 여유(일). 데이터 양 끝(또는 오늘) 바깥으로 이만큼 빈 칸을 둬서
// 막대를 끌어 옮길 자리를 만든다. '1개월'을 고정 30일로 본다(월말 경계 보정 불필요 — 목적이 여유 확보).
...
// ===== 간트차트 상태: 라벨 너비 (프로젝트별 localStorage) =====
const GANTT_LABELW_KEY = "personalGanttLabelWidths";
...
let ganttLabelWidths = loadObj(GANTT_LABELW_KEY); // { [projectId]: px }
...
function getLabelWidth() {
  const w = ganttLabelWidths[currentProjectId];
  return typeof w === "number" && w > 0 ? w : DEFAULT_LABEL_WIDTH;
}

// ----- 표시 기간 필터 (목록·간트 각각, 프로젝트별 localStorage) -----
const LIST_RANGES_KEY = "personalListRanges";
const GANTT_RANGES_KEY = "personalGanttRanges";
let listRanges = loadObj(LIST_RANGES_KEY); // { [projectId]: { start, end } }
...
function getListRange() {
  const r = listRanges[currentProjectId];
  return r && r.start && r.end ? r : null;
}
function getGanttRange() {
  const r = ganttRanges[currentProjectId];
  return r && r.start && r.end ? r : null;
}
...
function renderList() {
  listEl.innerHTML = "";
  if (!currentUid || !currentProjectId) {
...
// ----- 간트 확대/축소 (R-e) -----
// 조절 대상은 일자 칸 폭(--gantt-day-w) 하나뿐이다. 막대(칸마다 조각)·연월 밴드(span N)·
// 일자 헤더·세로 실선(칸 폭 상대 그라디언트)·주말 배경·가로 스크롤 범위(width:max-content)는
// 전부 칸 기준이라 폭만 바뀌면 브라우저가 알아서 재배치한다 → JS 재렌더 불필요.
// 값은 컨테이너(ganttEl)의 인라인 스타일에 둔다. renderGantt 가 innerHTML 을 비워도 ganttEl 은
// 살아남으므로, 드래그 중 잦은 재렌더(칸 경계마다)에도 배율이 기본값으로 되돌아가지 않는다.
// 저장하지 않는다 — 새로고침하면 기본 24px.
...
function zoomGantt(delta) {
  ...
  // 컨트롤 줄은 재렌더하지 않으므로 배율 표시·버튼 활성만 제자리에서 갱신
...
// 선택된 프로젝트의 할일을 간트차트로 렌더 (원본 수준, 드래그 편집 제외)
function renderGantt() {
  ...
  // 않게 한다. 최종 정렬은 드래그 종료 후 renderPersonalTasks()에서 한 번만 적용된다.
  if (ganttDragOrder) {
...
listToolbarEl.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".list-range-reset")) {
    delete listRanges[currentProjectId];
    saveListRanges();
    listRangeOpen = false;
    renderList();
    return;
  ...
  if (e.target.closest(".list-range-apply")) {
    ...
    listRanges[currentProjectId] = { start, end };
    saveListRanges();
    listRangeOpen = false;
    renderList();
    return;
...
// (얇은 경계선이라 손가락으로 스치기 쉬워, 터치는 반드시 롱프레스를 거친다)
...
function startLabelResize(pointerId, startX, isTouch) {
  ...
  const onMove = (ev) => {
    ...
    // (한 번 잡아두면 조절하던 폭이 저장 전 값으로 되돌아간다)
  ...
  const onUp = (ev) => {
    ...
    ganttLabelWidths[currentProjectId] = width;
...
// 날짜는 칸(축 단위) 경계에 스냅. 드래그 중에는 축을 고정하고, 끝나면 personalTasks 업데이트.
...
function startGanttDrag(taskId, mode, pointerId, startX, isTouch) {
  ...
  const onUp = (ev) => {
    ...
    if (moved) {
      ...
      renderPersonalTasks(); // 실제 축으로 다시 그림
      ganttEl.scrollLeft = keepScroll;
...
ganttToolbarEl.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".gantt-range-reset")) {
    delete ganttRanges[currentProjectId];
    saveGanttRanges();
    ganttRangeOpen = false;
    renderGantt();
    return;
  ...
  if (e.target.closest(".gantt-range-apply")) {
    ...
    ganttRanges[currentProjectId] = { start, end };
    saveGanttRanges();
    ganttRangeOpen = false;
    renderGantt();
    return;
...
listEl.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    finishEdit(); // 간트에서 들어온 수정이면 간트로 복귀
    clearMessage();
    // onSnapshot 만 믿지 않고 직접 재렌더 — 변경이 없거나 낙관적 스냅샷 타이밍 때문에
    // 수정 폼(저장/취소)이 닫히지 않는 문제 방지. (취소 경로와 동일하게 처리)
    renderPersonalTasks();
  } catch (e2) {
...
// ----- 선택된 프로젝트의 세부 할일 실시간 구독 -----
function subscribe(uid, projectId) {
  const q = query(
  ...
  unsubscribe = onSnapshot(
    ...
    (err) => {
      console.error("세부 할일 구독 실패:", err);
      showError(
        "할 일을 불러오지 못했습니다. Firestore 보안 규칙을 확인하세요: " +
          describeError(err)
      );
    }
  );
...
// 현재 사용자/프로젝트 조합으로 구독을 갱신(불필요한 재구독은 건너뜀).
function resubscribe() {
  const hasTarget = !!(currentUid && currentProjectId);
  detailEl.classList.toggle("has-project", hasTarget);

  const key = hasTarget ? `${currentUid}|${currentProjectId}` : null;
  ...
  if (hasTarget) {
    subscribe(currentUid, currentProjectId);
  } else {
    renderPersonalTasks();
  }
}

// ----- 프로젝트 선택 변경 수신(personalProjects.js 가 발행) -----
document.addEventListener(SELECT_EVENT, (e) => {
  const pid = (e.detail && e.detail.projectId) || null;
...
onAuthStateChanged(auth, (user) => {
  ...
  if (!user) currentProjectId = null; // 로그아웃 시 선택 해제
  resubscribe();
});
