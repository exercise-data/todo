...
// ===== 간트차트 상태: 축 단위·라벨 너비 (프로젝트별 localStorage) =====
const GANTT_UNITS_KEY = "personalGanttUnits";
...
let ganttUnits = loadObj(GANTT_UNITS_KEY); // { [projectId]: "auto"|"day"|"week"|"month" }
...
function getUnit() {
  return ganttUnits[currentProjectId] || "auto";
}
...
// 축 단위 선택기(+안내). 드래그 편집/기간 설정/내보내기는 이 단계 범위 밖.
...
// 축 단위 변경 → 프로젝트별 저장 후 재렌더
ganttEl.addEventListener("change", (e) => {
  const select = e.target.closest(".gantt-unit-select");
  if (!select || !currentProjectId) return;
  ganttUnits[currentProjectId] = select.value;
  saveGanttUnits();
  renderGantt();
});

// 라벨 열 너비 드래그 조절 (프로젝트별 저장). 칸은 1fr이라 라벨만 바꿔도 차트 폭 자동 조정.
